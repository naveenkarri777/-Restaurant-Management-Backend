const foodModal = require('../models/food-model');
...
const getFoodByResturantController = async (req, res) => {
  try {
    ...
    res.status(200).send({
      success: true,
      message: "Here is your requested food.",
      food,
    });
  } catch (error) {
    console.log(error);
    res.status(500).send({
      success: false,
      message: "Error In getFoodByResturantController API",
      error,
    });
  }
};
...
const placeOrderController = async (req, res) => {
  try {
    const newOrder = new orderModel({
      foods: req.body.foods.map((f) => f._id),
